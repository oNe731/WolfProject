...
public class Player_AttackRaMove : Player_Attack
{
    public Player_AttackRaMove(StateMachine<Player> stateMachine) : base(stateMachine, 1)
    {
        m_coolTime = 1f;
        m_animationName = "IsAttackRaMove";
    }
    ...
    private void Create_Projectile()
    {
        ...
        if (gameObject != null)
        {
            ...
            if (projectile != null)
            {
                projectile.Start_Projectile(m_effectPoint.position, m_owner.AttributeType, m_owner.Get_Direction(m_owner.Joystick.InputVector), m_owner.Get_Direction(), 2f);
            }
        }
    }
}
